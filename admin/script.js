...
function setupNavigation() {
    ...
    sidebarLinks.forEach((link, index) => {
        console.log(`🔗 Setting up link ${index + 1}:`, link.getAttribute('data-target'));
...
async function loadDashboard() {
    try {
        ...
        document.getElementById("cardProducts").innerText = productsData.success ? productsData.products.length : 0;
        document.getElementById("cardCategories").innerText = categoriesData.success ? categoriesData.categories.length : 0;
        document.getElementById("cardOrders").innerText = ordersData.success ? ordersData.orders.length : 0;
        document.getElementById("cardBlogs").innerText = blogsData.success ? blogsData.blogs.length : 0;
...
async function loadCategories() {
    try {
        ...
        const response = await fetch(`${API_BASE}/get_categories.php?shop_id=${currentShop.id}`);
...
async function loadProducts() {
    try {
        ...
        // Load products
        const response = await fetch(`${API_BASE}/get_my_products.php?shop_id=${currentShop.id}`);
        const data = await response.json();
        ...
        if (data.success && data.products.length > 0) {
            ...
            data.products.forEach(product => {
                const statusBadge = product.status === 'active' ? 'success' : 'secondary';
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-2">
                                    ${product.images && product.images.length > 0 ? 
                                        createProductGallery(product.images, product.id) :
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="product-badges">
                                        <span class="badge status-badge bg-${statusBadge}">${product.status}</span>
                                        ${product.is_new_arrival ? '<span class="badge new-arrival-badge">New Arrival</span>' : ''}
                                    </div>
...
async function loadOrders() {
    try {
        ...
        const response = await fetch(`${API_BASE}/get_orders.php?shop_id=${currentShop.id}`);
        const data = await response.json();
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-2">
                                    ${blog.image ? 
                                        `<img src="http://localhost/Indian%20Tribe/Backend_php/routes/uploads/blogs/${blog.image}" 
                                ...
                                <div class="col-md-8">
                                    ...
                                    <p class="text-muted mb-1">${blog.content ? blog.content.substring(0, 100) + '...' : 'No content'}</p>
...
async function deleteProduct(productId) {
    showPopup('Delete Product', 'Are you sure you want to delete this product? This action cannot be undone.', 'warning', async () => {
        try {
...
function setupEditControls(type) {
    ...
    document.getElementById(`${type}CancelBtn`).onclick = () => switchToAddMode(type);
}
...
async function deleteCategory(categoryId) {
    showPopup('Delete Category', 'Are you sure you want to delete this category?', 'warning', async () => {
        try {
...
function deleteBlog(blogId) {
    showPopup('Delete Blog', 'Are you sure you want to delete this blog post? This action cannot be undone.', 'warning', () => {
...
function createProductGallery(images, productId) {
    ...
    if (images.length === 1) {
        const imageUrl = `http://localhost/Indian%20Tribe/Backend_php/routes/uploads/products/${images[0]}`;
    ...
    displayImages.forEach((image, index) => {
        const imageUrl = `http://localhost/Indian%20Tribe/Backend_php/routes/uploads/products/${image}`;
        galleryHTML += `
...
function openImageModal(productId, startIndex = 0) {
    ...
    function updateModal() {
        const imageUrl = `http://localhost/Indian%20Tribe/Backend_php/routes/uploads/products/${product.images[currentIndex]}`;
