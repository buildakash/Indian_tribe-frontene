...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("http://localhost/Indian%20Tribe/Backend_php/routes/admin/login_admin.php", {
        method: "POST",
        body: formData
      });
      ...
      if (data.success) {
        ...
        if (window.sessionManager && window.sessionManager.saveSession(data.user)) {
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        showToast('❌ Cannot connect to server. Please check your internet connection or try again later.', 'error');
      } else if (err.message.includes('404')) {
        showToast('❌ Login service not found. Please contact administrator.', 'error');
        ...
        showToast('⚠️ An unexpected error occurred. Please try again later.', 'error');
      }
    } finally {
