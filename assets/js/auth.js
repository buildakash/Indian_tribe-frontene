...
class AuthSystem {
  ...
  setupOTPFormEvents() {
    ...
    if (resendBtn) {
      resendBtn.addEventListener("click", async () => {
        ...
        resendBtn.innerHTML =
          '<i class="ph ph-spinner animate-spin mr-1"></i>Resending...';
  ...
  showLoginForm() {
    ...
    const signupHeader = document.getElementById("signup-header");
    const otpHeader = document.getElementById("otp-header");
    const forgotPasswordHeader = document.getElementById(
      "forgot-password-header"
    );
    const resetPasswordHeader = document.getElementById(
  ...
  showResetOTPForm(email) {
    ...
    const resetOTPEmailDisplay = document.getElementById(
      "reset-otp-email-display"
    );
  ...
  async handleResetOTPVerification(email, otp) {
    ...
    try {
      ...
      const response = await fetch(
        `${this.apiBaseUrl}/verify_forgot_password_otp.php`,
        {
          method: "POST",
          body: formData,
        }
  ...
  setupEventListeners() {
    ...
    const showSignupBtnRight = document.getElementById(
      "show-signup-form-right"
    );
    ...
    const backToLoginFromForgotBtn = document.getElementById(
    ...
    const backToForgotPasswordBtn = document.getElementById(
      "back-to-forgot-password"
    );
    if (backToForgotPasswordBtn) {
      backToForgotPasswordBtn.addEventListener("click", () =>
        this.showForgotPasswordForm()
      );
    }

    // Back to forgot password from reset OTP
    const backToForgotFromResetOTPBtn = document.getElementById(
      "back-to-forgot-from-otp"
    );
    if (backToForgotFromResetOTPBtn) {
      backToForgotFromResetOTPBtn.addEventListener("click", () =>
        this.showForgotPasswordForm()
      );
    }

    // Back to forgot password from reset OTP
    const backToForgotFromOTPBtn = document.getElementById(
      "back-to-forgot-from-otp"
    );
    if (backToForgotFromOTPBtn) {
      backToForgotFromOTPBtn.addEventListener("click", () =>
        this.showForgotPasswordForm()
      );
    }
    ...
    strengthIndicator.innerHTML = `
            ...
            <div class="space-y-1 text-xs bg-gray-50 p-3 rounded-lg border">
                ...
                ${feedback
                  .map(
                    (item) => `<div class="${
                      item.startsWith("✓") ? "text-green-600" : "text-red-600"
                    } flex items-center">
                    <span class="mr-2">${
                      item.startsWith("✓") ? "✓" : "✗"
                    }</span>
      ...
      if (input) {
        ...
        input.addEventListener("paste", (e) => {
          ...
          const nextEmptyIndex =
            numbers.length < inputs.length ? numbers.length : inputs.length - 1;
          const focusIndex =
            numbers.length === inputs.length ? lastFilledIndex : nextEmptyIndex;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(() => {
    ...
    if (window.authSystem) {
      console.log(
        "AuthSystem methods:",
        Object.getOwnPropertyNames(Object.getPrototypeOf(window.authSystem))
      );
    }
  }, 1000);
});
