...
if (!productContainer || !productList || !listPagination) {
    console.error("Shop product containers not found. Check if product-list.html loaded correctly.");
} else {
    let currentPage = 1;
    ...
    function fetchProducts() {
        // IMPORTANT: Ensure this path is correct relative to shop.html
        fetch('../assets/data/Product.json')
            ...
            .catch(error => {
                ...
                if(productList) productList.innerHTML = `<p class="col-span-full text-center text-red-500">Error: Could not load product data. Please check the path and file format.</p>`;
            });
    }
    ...
    function renderProducts(page, products = []) {
        ...
        displayedProducts.forEach(product => {
            ...
            product.thumbImage.forEach((img, index) => {
                // Adjust path for component structure
                let imagePath = `../${img.replace('./', '')}`;
                productImages += `<img key="${index}" class="w-full h-full object-cover duration-700" src="${imagePath}" alt="img">`;
            });
            
            if (!productContainer.classList.contains('style-list')) { // Default to grid
                        ...
                        <div class="product-infor mt-4 lg:mb-7">
                             <div class="product-sold sm:pb-4 pb-2">
                                <div class="progress bg-line h-1.5 w-full rounded-full overflow-hidden relative">
                                    <div class='progress-sold bg-red absolute left-0 top-0 h-full' style="width: ${Math.floor((product.sold / product.quantity) * 100)}%"></div>
                                </div>
                                <div class="flex items-center justify-between gap-3 gap-y-1 flex-wrap mt-2">
                                    <div class="text-button-uppercase"><span class='text-secondary2 max-sm:text-xs'>Sold: </span><span class='max-sm:text-xs'>${product.sold}</span></div>
                                    <div class="text-button-uppercase"><span class='text-secondary2 max-sm:text-xs'>Available: </span><span class='max-sm:text-xs'>${product.quantity - product.sold}</span></div>
                                </div>
                            </div>
                            ...
                            <div class="product-price-block flex items-center gap-2 flex-wrap mt-1 duration-300 relative z-[1]">
                                ...
                                <div class="product-sale caption1 font-medium bg-green px-3 py-0.5 inline-block rounded-full">-${Math.floor(100 - ((product.price / product.originPrice) * 100))}%</div>
                                ` : ''}
                            </div>
                        </div>
                    </div>`;
                ...
                productItem.innerHTML = `
                    <div class="product-main flex gap-5 items-start" data-item="${product.id}">
                        ...
                        <div class="content flex-1">
                            ...
                            <div class="product-sold sm:pb-3 pb-2 mt-3">
                                ...
                                <div class="flex items-center gap-4 mt-2">
                                    <div class="text-button-uppercase"><span class='text-secondary2'>Sold: </span>${product.sold}</div>
                                    <div class="text-button-uppercase"><span class='text-secondary2'>Available: </span>${product.quantity - product.sold}</div>
                                </div>
                            </div>
    ...
    function handleFiltersChange() { /* ... same as before ... */ }
    function addEventListenersToFilters() { /* ... same as before ... */ }
    function renderPagination(products = []) { /* ... same as before ... */ }
