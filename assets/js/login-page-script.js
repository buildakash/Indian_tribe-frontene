...
(function () {
  ...
  function initializeLoginLogic() {
    ...
    const API_BASE = 'http://localhost/Indian%20Tribe/backend_php/routes/auth';
    const ROUTES = {
    ...
    function checkConfirmPassword() {
      ...
      if (pass === confirmPass) {
          confirmIndicator.innerHTML = '<span class="text-green-500 flex items-center text-xs"><i class="ph ph-check-circle mr-1"></i> Passwords match</span>';
      } else {
          confirmIndicator.innerHTML = '<span class="text-red-500 flex items-center text-xs"><i class="ph ph-x-circle mr-1"></i> Passwords do not match</span>';
      }
    }
    ...
    btnToggleLoginPass?.addEventListener('click', () => togglePasswordVisibility(inputLoginPass, btnToggleLoginPass));
    btnToggleResetPass?.addEventListener('click', () => togglePasswordVisibility(inputNewPass, btnToggleResetPass));
    btnToggleResetConfirm?.addEventListener('click', () => togglePasswordVisibility(inputConfirmPass, btnToggleResetConfirm));
    ...
    forgotForm.addEventListener('submit', async e => {
      ...
      try {
        ...
        startCountdown(120);
        switchTo(otpForm);
        window.toast.success(resp.message || 'Verification code sent to your email.');
      } catch (err) {
    ...
    btnResendOtp?.addEventListener('click', async () => {
      if (!isValidEmail(resetEmail)) { window.toast.error('Missing email. Please go back and enter your email again.'); return; }
    ...
    resetForm.addEventListener('submit', async e => {
      ...
      if (!strength.allMet) {
          window.toast.error('Your new password does not meet all the requirements.');
          return;
      }
      ...
      try {
        ...
        window.toast.success(resp.message || 'Password updated successfully! You can now log in.');
